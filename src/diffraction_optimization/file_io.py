...
def load_digit_images_from_dataset(dataset: pd.DataFrame, digit: int, load_amt=None) -> np.ndarray:
    ...
    for index in range(vectors_amt):
        digit_matrix[:, :, index] = parse_mnist_digit_to_matrix(digit_vectors.iloc[index])
    return digit_matrix
